...
public class ContourDetectControl : OpenCvPreviewNodeControl
{
    ...
    private readonly ContourDetectProcessor _processor;

    public ContourDetectControl(ContourDetectProcessor processor)
    ...
    private void OnResultUpdated(Mat? image, IReadOnlyList<ContourInfo> contours)
    {
        SetPreviewImage(
            image,
            image == null || image.Empty()
                ? "No Image"
                : CreateStatusText(image, contours));
    }

    private string CreateStatusText(Mat image, IReadOnlyList<ContourInfo> contours)
    {
        return
            $"{image.Width} x {image.Height} / " +
            $"Count={contours.Count} / " +
